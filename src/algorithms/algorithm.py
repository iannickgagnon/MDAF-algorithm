...
class Algorithm(ABC):
        ...
        match attribute_name:
            ...
            case (
                ...
                raise AttributeError(
                    "\033[91mInitial, current and best values are updated automatically"
            ...
            case "current_solution":
                ...
                self.context.value_history.append(
                    self.context.objective(self.current_solution)
                )
                ...
                if self.context.current_value < self.context.best_value:
                    ...
                    self.context.first_hitting_time = (
                        len(self.context.value_history) - 1
                    )
    ...
    def initialize_solutions_and_values(self) -> None:
        ...
        if self.context.solution_initializer is not None:
            ...
            raise AttributeError(
                "\033[91mPlease provide an initial solution (initial_solution: list | np.ndarray)"
        ...
        if self.context.nb_agents is None:
            ...
            if isinstance(self.context.initial_solution, (int, float, list)):
                ...
                raise TypeError(
                    "\033[91mInitial solution (initial_solution) must be a list, an int, a float, or a NumPy array.\033[0m"
                )

            # Warn user
            print(
                f"\033[93mNumber of agents not provided. Automatically set to {self.context.nb_agents} based on initial solution size.\033[0m"
            )
    ...
    def plot_profile(
        ...
        # Validate
        assert (
            metric in self.profiles.__dict__.values()
        ...
        bootstrap_results = bootstrap(
            (histories,), np.mean, n_resamples=nb_bootstrap_samples, method="percentile"
        )
    ...
    def generate_metric_distribution(
        ...
        # Validate
        assert (
            metric in self.metrics.__dict__.values()
        ...
        for _ in range(sample_size):
            ...
            # Collect
            metrics.append(
                algo.best_value if metric == "best" else algo.first_hitting_time
            )
    ...
    def generate_distribution(
        ...
        if metric and foo_statistic:
            raise AttributeError(
                "\033[91mPlease provide either a metric or a statistic function, not both.\033[0m"
            )
        ...
        if foo_statistic is None:
            ...
            sample = self.generate_statistic_distribution(
                metric=metric,
    ...
    def plot_distribution(
        ...
        # Generate the sample
        sample = self.generate_distribution(
            metric=metric,
            foo_statistic=foo_statistic,
            sample_size=sample_size,
            sub_sample_size=sub_sample_size,
        ...
        bootstrap_samples = np.random.choice(
            sample, (nb_bootstraps, sample_size), replace=True
        )
        bootstrap_hist = np.array(
            [np.histogram(bs, bins=bins, density=True)[0] for bs in bootstrap_samples]
        )
